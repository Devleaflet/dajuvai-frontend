                    ...
                    <div className="attribute-values">
                        {attr.values.map((val, valIndex) => (
                            <div key={valIndex} className="attribute-value">
                                ...
                                {/* Nested attributes */}
                                {val.nestedAttributes?.map(
                                    (nested, nestedIndex) => (
                                        <div
                                            key={nestedIndex}
                                            className="nested-attribute"
                                        >
                                            <input
                                                value={nested.type}
                                                onChange={(e) => {
...
const NewProductModal: React.FC<NewProductModalProps> = ({
        ...
        if (field === "discount" || field === "discountAmount" || field === "discountPercent" || field === "discountType") {
            setFormData((prev) => {
                ...
                if (field === "discountType") {
                    discountType = hasType
                        ? (next.discountType as "FLAT" | "PERCENTAGE")
                        : "NONE";
                } else {
                    discountType = hasType
                        ? (next.discountType as "FLAT" | "PERCENTAGE")
    ...
    const handleImageDrop = (e: React.DragEvent<HTMLDivElement>) => {
        ...
        if (dt && dt.files && dt.files.length > 0) {
            const droppedFiles = Array.from(dt.files).filter((f) =>
            ...
            if (droppedFiles.length > 0) {
                ...
                setCurrentStep(3);
                toast(
                    `${droppedFiles.length} image${droppedFiles.length > 1 ? "s" : ""} added`,
                );
            }
            dt.clearData();
        }
    ...
    const generateVariants = () => {
        ...
        const newVariants: ProductVariant[] = combos.map((combo, idx) => {
            ...
            const skuParts: string[] = combo.map((p) =>
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            productData.discountType = normalizeDiscountType(
                formData.discountType,
            );
      ...
      `}</style>
            ...
            >
                <div className="new-product-modal-header">
                    <h2 className="new-product-modal-title">
                        Create New Product
                    </h2>
                    <button
