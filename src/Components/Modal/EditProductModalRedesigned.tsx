...
const EditProductModal: React.FC<EditProductModalProps> = ({
    ...
    const [attributeSpecs, setAttributeSpecs] = useState<
        Array<{ type: string; valuesText: string }>
    >([{ type: "", valuesText: "" }]);
    ...
    const formatVariantAttributes = (attributes: any): string => {
        ...
        if (Array.isArray(attributes)) {
            return attributes
                .map((attr: any) => {
                    const label = String(
                        attr?.type ?? attr?.attributeType ?? "",
                    ).trim();
                    const valuesSrc =
                        attr?.values ?? attr?.attributeValues ?? [];
                    const vals = (
                        Array.isArray(valuesSrc) ? valuesSrc : [valuesSrc]
                    )
                        .map((v: any) => {
                            if (v == null) return "";
                            if (typeof v === "object")
            ...
            return Object.entries(attributes)
                .map(([key, value]) => {
                    ...
                    if (typeof value === "object") {
                        const val =
                            (value as any).value ?? (value as any).name ?? "";
                        return val
                            ? `${key}: ${String(val)}`
    ...
    const generateVariants = () => {
        const cleanSpecs = attributeSpecs
            .map((s) => ({
                type: s.type.trim(),
                values: parseValues(s.valuesText),
            }))
        ...
        try {
            ...
            if (!cats || cats.length === 0) {
                try {
                    ...
                    console.warn(
                        "Failed to fetch categories while resolving category from subcategory:",
                        e,
                    );
                    return null;
                }
            ...
            for (const cat of cats) {
                try {
                    ...
                    if (
                        Array.isArray(subs) &&
                        subs.some((s: any) => Number(s.id) === Number(subId))
                    ) {
                        return { categoryId: cat.id, subcategories: subs };
                    }
                } catch (e) {
                    console.warn(
                        `Failed to fetch subcategories for category ${cat.id}:`,
                        e,
                    );
                }
            }
            ...
            console.warn(
                "Unexpected error resolving category from subcategory:",
                e,
            );
        }
        return null;
    ...
    const populateFormWithProduct = async () => {
        ...
        if (!categoryId && subcategoryId) {
            console.warn(
                "ℹ️ Category ID missing; resolving from subcategory...",
            );
            const resolved =
                await resolveCategoryFromSubcategoryId(subcategoryId);
            if (resolved) {
        ...
        try {
            ...
            console.warn(
                "⚠️ Failed to fetch latest product by ID. Falling back to provided product. Error:",
                e,
            );
        }
        ...
        const normalizeVariantAttributes = (raw: any): Attribute[] => {
            ...
            if (typeof raw === "object" && !Array.isArray(raw)) {
                return Object.entries(raw).map(([key, value]) => ({
                    type: String(key),
                    values: (Array.isArray(value) ? value : [value]).map(
                        (val: any) => ({
                            value: String(
            ...
            if (Array.isArray(raw)) {
                return (raw as any[])
                    .map((attr) => {
                        ...
                        const valsSrc =
                            attr?.values ?? attr?.attributeValues ?? [];
                        const valsArr = Array.isArray(valsSrc)
                            ? valsSrc
                            : [valsSrc];
                    ...
                    .filter(
                        (a: Attribute) =>
                            a.type && a.values && a.values.length > 0,
                    );
            }
            return [];
        };
        ...
        if (Array.isArray(fullProduct.productImages)) {
            resolvedProductImages = (fullProduct.productImages as any[])
                .map(pickUrl)
                .filter(Boolean);
        } else if (Array.isArray(fullProduct.images)) {
            resolvedProductImages = (fullProduct.images as any[])
                .map(pickUrl)
                .filter(Boolean);
        } else if (Array.isArray(fullProduct.gallery)) {
            resolvedProductImages = (fullProduct.gallery as any[])
                .map(pickUrl)
                .filter(Boolean);
        } else if (Array.isArray(fullProduct.media)) {
            resolvedProductImages = (fullProduct.media as any[])
                .map(pickUrl)
                .filter(Boolean);
        } else if (fullProduct.image) {
            resolvedProductImages = [pickUrl(fullProduct.image)].filter(
                Boolean,
            );
        }
        ...
        setFormData({
            ...
            categoryId: Number(
                categoryId || fullProduct.categoryId || selectedCategoryId || 0,
            ),
            subcategoryId: Number(
                subcategoryId || fullProduct.subcategory?.id || 0,
            ),
            quantity: Number(fullProduct.quantity ?? fullProduct.stock ?? 0),
            brand_id: fullProduct.brand_id ?? null,
            dealId: fullProduct.dealId ?? null,
            inventory: Array.isArray(fullProduct.inventory)
                ? fullProduct.inventory
                : [],
            vendorId: String(
                fullProduct.vendorId ?? fullProduct.vendor?.id ?? "",
            ),
    ...
    const addAttribute = (variantIndex: number) => {
        if (newAttribute.type && newAttribute.values[0]?.value) {
            ...
            const normalized: Attribute = {
                ...
                values: newAttribute.values.map((v) => ({
                    value: v.value,
                    nestedAttributes: v.nestedAttributes || [],
                })),
            };
    ...
    const validateForm = (): string | null => {
        ...
        if (!formData.hasVariants) {
            ...
            for (const variant of variants) {
                ...
                if (discType === "PERCENTAGE" && variant.discountPercent !== undefined && variant.discountPercent !== null && Number(variant.discountPercent) > 0) {
                    if (Number(variant.discountPercent) > 100) {
                        return `Variant (${variant.sku}): Percentage discount cannot exceed 100%`;
                    }
                } else if (discType === "FLAT" && variant.discountAmount !== undefined && variant.discountAmount !== null && Number(variant.discountAmount) > 0) {
                    if (Number(variant.discountAmount) > Number(variant.price || 0)) {
                        return `Variant (${variant.sku}): Discount amount cannot be greater than the base price`;
                    }
                }
            }
        }
        ...
        const discountValue = formData.discountType === "PERCENTAGE" ? formData.discountPercent : formData.discountAmount;
        if (
            formData.discountType &&
            formData.discountType !== "NONE" &&
            (!discountValue || Number(discountValue) <= 0)
        ) {
            return "Please enter a discount amount/percentage when a discount type is selected";
        }
        if (
            ...
            if (!formData.discountType || formData.discountType === "NONE")
                return "Please select a discount type (Percentage or Flat)";

            const discType = normalizeDiscountType(formData.discountType);
            if (discType === "PERCENTAGE" && Number(discountValue) > 100) {
            ...
            if (discType === "FLAT") {
                const checkPrice = !formData.hasVariants
                    ...
                    : variants.length > 0
                      ...
                      : 0;
                if (Number(discountValue) > checkPrice && checkPrice > 0) {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            // Send both fields to match the Product entity's columns.
            // Backend derives the counterpart value (flat <-> percent) from whichever one the vendor actually filled in.
            updatePayload.discountAmount =
                formData.discountAmount != null ? Number(formData.discountAmount) : 0;
            updatePayload.discountPercent =
                formData.discountPercent != null ? Number(formData.discountPercent) : 0;
            updatePayload.discountType = normalizeDiscountType(
                formData.discountType,
            );
                ...
                <div className="new-product-modal-body">
                    <form onSubmit={handleSubmit} className="new-product-form">
                        ...
                        <div className="form-section">
                            ...
                            >
                                <div
                                    className={`toggle-switch ${formData.hasVariants ? "active" : ""}`}
                                >
                                ...
                                <div>
                                    ...
                                    <div className="toggle-description">
                                        Enable this if your product comes in
                        ...
                        {!formData.hasVariants &&
                            (() => {
                                ...
                                return (
                                    <div
                                        className="form-section price-summary-section"
                                        style={{
                                    ...
                                    {attributeSpecs.map((spec, i) => (
                                        <React.Fragment key={i}>
                                            ...
                                            <div className="form-group">
                                                ...
                                                />
                                                <div className="label-hint">
                                                    Tip: press comma to separate
                                                    values. Duplicates are
                                                    ignored.
                                                </div>
                                                {parseValues(spec.valuesText)
                                                    .length > 0 && (
                                                ...
                                                <div className="form-grid three-columns">
                                                    ...
                                                    <div className="form-group">
                                                        ...
                                                        <input
                                                            ...
                                                            className="form-input"
                                                            value={
                                                                (variant.discountType === 'PERCENTAGE' ? variant.discountPercent : variant.discountAmount) ??
                                                                ""
                                                            }
                                                            onChange={(e) => {
                                                                const val = e.target.value === "" ? 0 : Number(e.target.value);
                                                                if (variant.discountType === 'PERCENTAGE') {
                                                                    updateVariant(index, "discountPercent", val);
                                                                } else {
