...
const ArchivedProductDetailModal: React.FC<ArchivedProductDetailModalProps> = ({
  ...
  const [confirmAction, setConfirmAction] = useState<
    { type: "product" } | { type: "variants"; ids: number[] } | null
  >(null);
  ...
  const runRestoreVariants = async (ids: number[]) => {
    ...
    try {
      ...
      toast.success(
        ids.length > 1 ? `${ids.length} variants restored` : "Variant restored",
      );
  ...
  return (
    <div className="new-product-modal-overlay">
      ...
      <style>{`
                ...
                .archived-detail-description::-webkit-scrollbar { width: 8px; }
                .archived-detail-description::-webkit-scrollbar-track { background: transparent; }
                .archived-detail-description::-webkit-scrollbar-thumb { background: #d1d5db; border-radius: 999px; }
                .archived-detail-description::-webkit-scrollbar-thumb:hover { background: #9ca3af; }
            `}</style>
      <div className="new-product-modal" onClick={(e) => e.stopPropagation()}>
        <div className="new-product-modal-header">
          ...
          <button className="new-product-modal-close" onClick={onClose}>
        ...
        <div className="new-product-modal-body">
          <div
            ...
            style={{
              background: item.isProductArchived ? "#fef2f2" : "#fffbeb",
              border: `1px solid ${
          ...
          >
            <p
              style={{
                margin: 0,
                fontWeight: 600,
                color: item.isProductArchived ? "#b91c1c" : "#b45309",
              }}
            >
              {item.isProductArchived
            ...
            <div className="form-grid two-columns">
              ...
              {item.description && (
                <div className="form-group full-width">
                  ...
                  <div className="form-input archived-detail-description">
                    {item.description}
                  </div>
                </div>
              )}
            ...
            <div className="form-section">
              ...
              {item.productImages?.length > 0 && (
                <div className="image-preview-grid">
                  {item.productImages.map((url: string, i: number) => (
          ...
          {item.hasVariants && (
            <div className="form-section">
                ...
                <div key={variant.id} className="variant-card">
                  <div className="variant-header">
                    <div className="variant-title">
                      ...
                      {formatAttributes(variant.attributes) || variant.sku}
                      <span
                  ...
                  {variant.variantImages?.length > 0 && (
                    <div className="image-preview-grid">
                      {variant.variantImages.map((url: string, i: number) => (
                        <div key={i} className="image-preview">
                          <img src={url} alt="" />
                        </div>
                      ))}
              ...
              {archivedVariants.map((variant: any, index: number) => (
                <div
                  key={variant.id}
          ...
          <div className="button-group" style={{ padding: 0 }}>
            ...
            {item.isProductArchived ? (
              <button
                ...
                disabled={isRestoring}
                onClick={() => setConfirmAction({ type: "product" })}
              >
                Restore Product
              </button>
            ) : (
              <button
                type="button"
                className="btn btn-primary"
                disabled={isRestoring || selectedVariantIds.size === 0}
                onClick={() =>
                  setConfirmAction({
                    type: "variants",
