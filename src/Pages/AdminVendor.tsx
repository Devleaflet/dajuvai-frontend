...
import { Vendor, District, ApiResponse, VendorUpdateRequest } from '../Components/Types/vendor';
...
const SkeletonRow: React.FC = () => {
  return (
    <tr className="admin-vendors__table-row">
      <td><div className="admin-vendors__skeleton"></div></td>
      <td><div className="admin-vendors__skeleton"></div></td>
      <td><div className="admin-vendors__skeleton"></div></td>
      <td><div className="admin-vendors__skeleton"></div></td>
      <td><div className="admin-vendors__skeleton"></div></td>
...
const createVendorAPI = (token: string | null) => ({
  async getAll(): Promise<Vendor[]> {
    try {
      ...
      return (result.data || []).map((vendor) => ({
        ...
        taxDocuments: Array.isArray(vendor.taxDocuments) ? vendor.taxDocuments : vendor.taxDocuments ? [vendor.taxDocuments] : [],
        businessRegNumber: vendor.businessRegNumber || 'N/A',
        citizenshipDocuments: Array.isArray(vendor.citizenshipDocuments) ? vendor.citizenshipDocuments : vendor.citizenshipDocuments ? [vendor.citizenshipDocuments] : [],
        chequePhoto: Array.isArray(vendor.chequePhoto) ? vendor.chequePhoto : vendor.chequePhoto ? [vendor.chequePhoto] : [],
  ...
  async getUnapproved(): Promise<Vendor[]> {
    try {
      ...
      console.log(`Fetch finished loading: GET "${API_BASE_URL}/api/vendors/unapprove/list"`);
  ...
  async approve(id: number): Promise<void> {
    try {
      ...
      console.log(`Fetch finished loading: PUT "${API_BASE_URL}/api/vendors/approve/${id}"`);
  ...
  async reject(id: number): Promise<void> {
    try {
      ...
      console.log(`Fetch finished loading: PUT "${API_BASE_URL}/api/vendors/reject/${id}"`);
  ...
  async update(id: number, vendorData: Partial<VendorUpdateRequest>): Promise<Vendor> {
    try {
      if (!token) {
        throw new Error('No token provided. Please log in.');
      }
      ...
      console.log('Sending PUT request with normalized data:', normalizedVendorData);
      ...
      console.log(`Fetch finished loading: PUT "${API_BASE_URL}/api/vendors/${id}"`);
      ...
      if (!result.data) {
        return {
          ...
          citizenshipDocuments: Array.isArray(normalizedVendorData.citizenshipDocuments)
...
const ConfirmationModal: React.FC<ConfirmationModalProps> = ({ show, onClose, onConfirm, action }) => {
  if (!show) return null;
...
const AdminVendor: React.FC = () => {
  ...
  const loadVendors = useCallback(async () => {
    try {
      ...
      localStorage.setItem(CACHE_KEY_VENDORS, JSON.stringify({ data: allVendors, timestamp: Date.now() }));
    } catch (err) {
  ...
  useEffect(() => {
    ...
    const loadInitialData = async () => {
      await Promise.all([loadVendors(), fetchDistricts(), loadUnapprovedCount()]);
    };

    loadInitialData();
  }, [isAuthenticated, token, fetchDistricts, loadUnapprovedCount, loadVendors, navigate]);

  // Auto-filter when approval filter changes
  useEffect(() => {
    handleFilter();
  }, [approvalFilter, districtFilter, statusFilter, startDate, endDate, vendors]);
  ...
  const handleFilter = () => {
    ...
    if (approvalFilter !== 'all') {
      if (approvalFilter === 'approved') {
        ...
        filtered = filtered.filter((vendor) => vendor.isApproved === false || vendor.isApproved === null);
      }
    }
  ...
  const handleSaveVendor = async (vendorData: Partial<VendorUpdateRequest>) => {
    ...
    try {
      ...
      const updatedVendor = await vendorAPI.update(selectedVendor.id, normalizedVendorData);
      setVendors((prev) =>
  ...
  const handleConfirm = async () => {
    ...
    try {
      if (confirmAction === 'approve') {
        ...
        setVendors((prev) =>
          prev.map((v) => (v.id === selectedVendorId ? { ...v, isApproved: true, status: 'Active' } : v))
        );
        setFilteredVendors((prev) =>
          prev.map((v) => (v.id === selectedVendorId ? { ...v, isApproved: true, status: 'Active' } : v))
        );
        ...
        setFilteredVendors((prev) => prev.filter((v) => v.id !== selectedVendorId));
      ...
      toast.error(error instanceof Error ? error.message : `Failed to ${confirmAction} vendor`);
    } finally {
    ...
    <div className="admin-vendors">
      ...
      <div>
        <div className="admin-vendors__content">
          ...
          <div className="admin-vendors__search-container">
            <input
              type="text"
              placeholder="Search vendors..."
              value={searchQuery}
              onChange={(e) => handleSearch(e.target.value)}
              className="admin-vendors__search-input"
            />
          </div>
              ...
              <table className="admin-vendors__table">
                ...
                <tbody>
                  {loading ? (
                    ...
                    <tr>
                      <td colSpan={7} className="admin-vendors__table-row">
                        No vendors found
                      </td>
                    </tr>
                  ) : (
                    currentVendors.map((vendor) => (
                      <tr key={vendor.id} className="admin-vendors__table-row">
                        ...
                        <td className="admin-vendors__status-column">{vendor.status}</td>
                        <td className="admin-vendors__approval-column">
                          <span className={`approval-status ${vendor.isApproved ? 'approved' : 'pending'}`}>
