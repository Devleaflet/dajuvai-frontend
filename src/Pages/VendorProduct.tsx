...
const VendorProduct: React.FC = () => {
  ...
  const { data: vendorStats } = useQuery({
    ...
    enabled: !!authState.vendor?.id && !!authState.token,
    queryFn: () =>
      VendorDashboardService.getInstance().getVendorStats(authState.token!),
  });

  const products: Product[] = productData?.products || [];
  const totalProducts = productData?.serverTotal || productData?.total || 0;
  const lowStockCount = vendorStats?.lowStockCount || 0;
  const outOfStockCount = vendorStats?.outOfStockCount || 0;
  ...
  return (
    <div className="vendor-dash-container">
      ...
      <div className={`dashboard ${isMobile ? "dashboard--mobile" : ""}`}>
        ...
        >
          {!outOfStockBannerDismissed && outOfStockCount > 0 && (
            <div className="low-stock-banner low-stock-banner--out">
              ...
              <p className="low-stock-banner__msg">
                You have <strong>{outOfStockCount}</strong> product
                {outOfStockCount > 1 ? "s" : ""} out of stock. Restock to
                avoid missed sales.
              </p>
              <button
                className="low-stock-banner__close"
                onClick={() => setOutOfStockBannerDismissed(true)}
                aria-label="Dismiss"
              >
                ×
              </button>
          ...
          {!lowStockBannerDismissed && lowStockCount > 0 && (
            <div className="low-stock-banner low-stock-banner--low">
              <span className="low-stock-banner__icon">
                <svg
                  width="18"
                  height="18"
              ...
              <p className="low-stock-banner__msg">
                You have <strong>{lowStockCount}</strong> product
                {lowStockCount > 1 ? "s" : ""} low on stock. Consider
                restocking soon.
              </p>
              <button
                className="low-stock-banner__close"
