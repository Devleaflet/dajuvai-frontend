...
const AdminDistrict: React.FC = () => {
  ...
  const [showEditModal, setShowEditModal] = useState<{ show: boolean; district: District | null }>({ show: false, district: null });
  const [showDeleteModal, setShowDeleteModal] = useState<{ show: boolean; district: District | null }>({ show: false, district: null });
  ...
  const handleAddDistrict = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.ok && data.success) {
        ...
        localStorage.setItem(CACHE_KEY, JSON.stringify({ data: [...districts, data.data], timestamp: Date.now() }));
  ...
  const handleEditDistrict = async (e: React.FormEvent) => {
    ...
    try {
      const response = await fetch(`${API_BASE_URL}/api/district/${showEditModal.district.id}`, {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json',
          'Accept': 'application/json',
      ...
      const data = await response.json();
      if (response.ok && data.success) {
        setDistricts((prev) => prev.map(d => d.id === data.data.id ? data.data : d));
        localStorage.setItem(CACHE_KEY, JSON.stringify({ data: districts.map(d => d.id === data.data.id ? data.data : d), timestamp: Date.now() }));
  ...
  const handleDeleteDistrict = async () => {
    ...
    try {
      const response = await fetch(`${API_BASE_URL}/api/district/${showDeleteModal.district.id}`, {
        method: 'DELETE',
        headers: {
          'Content-Type': 'application/json',
          'Accept': 'application/json',
      ...
      if (response.ok) {
        setDistricts((prev) => prev.filter(d => d.id !== showDeleteModal.district!.id));
        localStorage.setItem(CACHE_KEY, JSON.stringify({ data: districts.filter(d => d.id !== showDeleteModal.district!.id), timestamp: Date.now() }));
  ...
  const paginatedDistricts = filteredDistricts.slice((currentPage - 1) * PAGE_SIZE, currentPage * PAGE_SIZE);

  const renderSkeleton = () => (
    Array.from({ length: PAGE_SIZE }).map((_, index) => (
      <tr key={index} className="admin-district__skeleton-row">
        <td><div className="admin-district__skeleton admin-district__skeleton-id"></div></td>
        <td><div className="admin-district__skeleton admin-district__skeleton-name"></div></td>
        <td>
      ...
      <div className="admin-district__content">
        ...
        <div className="admin-district__header-row">
          ...
          <button className="admin-district__add-btn" onClick={() => setShowAddModal(true)}>
            <FiPlus style={{ marginRight: 8, verticalAlign: 'middle' }} /> Add District
          </button>
        </div>
        <div className="admin-district__search-row">
          <input
            className="admin-district__search-input"
            type="text"
          ...
          {error ? (
            ...
            <>
              <table className="admin-district__table">
                ...
                <tbody>
                  {loading ? renderSkeleton() : paginatedDistricts.map((district) => (
                    <tr key={district.id}>
                      <td>{district.id}</td>
                      <td>{district.name}</td>
                      <td>
                        <button
                          ...
                          title="Edit"
                          onClick={() => { setShowEditModal({ show: true, district }); setEditDistrictName(district.name); }}
                        >
                          <FiEdit2 />
                        </button>
                        <button
                          ...
                          title="Delete"
                          onClick={() => setShowDeleteModal({ show: true, district })}
                        >
                          <FiTrash2 />
                        </button>
                      </td>
                  ...
                  {!loading && paginatedDistricts.length === 0 && (
                    <tr><td colSpan={3} style={{ textAlign: 'center', color: '#888' }}>No districts found.</td></tr>
                  )}
                </tbody>
              </table>
              {totalPages > 1 && (
                <div className="admin-district__pagination">
                  ...
                  {Array.from({ length: totalPages }).map((_, i) => (
                    <button
                      key={i}
                      className={`admin-district__pagination-btn${currentPage === i + 1 ? ' active' : ''}`}
            ...
            <div className="admin-district__modal">
              ...
              <form onSubmit={handleEditDistrict}>
                ...
                <div className="admin-district__modal-actions">
                  ...
                  <button type="button" onClick={() => setShowEditModal({ show: false, district: null })}>Cancel</button>
                </div>
              </form>
            </div>
          </div>
        ...
        {showDeleteModal.show && showDeleteModal.district && (
          <div className="admin-district__modal-overlay">
            <div className="admin-district__modal">
              ...
              <p>Are you sure you want to delete <b>{showDeleteModal.district.name}</b>?</p>
              <div className="admin-district__modal-actions">
                <button onClick={handleDeleteDistrict}>Delete</button>
                <button onClick={() => setShowDeleteModal({ show: false, district: null })}>Cancel</button>
              </div>
            </div>
          </div>
        )}
