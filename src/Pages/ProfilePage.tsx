...
const ProfilePage: React.FC = () => {
  ...
  const [vendorDetails, setVendorDetails] = useState<VendorProfile | null>(null);
  const [originalDetails, setOriginalDetails] = useState<VendorProfile | null>(null);
  ...
  const getAvatarColor = (businessName: string) => {
    ...
    const charCodeSum = businessName.split("").reduce((sum, char) => sum + char.charCodeAt(0), 0);
  ...
  const validateBusinessName = (businessName: string) => businessName.trim().length >= 2;
  const validatePhoneNumber = (phoneNumber: string) => phoneNumber === "" || /^[0-9]{10}$/.test(phoneNumber);
  ...
  const fetchVendorDetails = async () => {
    ...
    try {
      const response = await axios.get(`${API_BASE_URL}/api/vendors/auth/vendor`, {
  ...
  const handleFileChange = async (e: React.ChangeEvent<HTMLInputElement>, field: keyof VendorProfile) => {
  ...
  const handleArrayFileChange = async (e: React.ChangeEvent<HTMLInputElement>, field: keyof VendorProfile) => {
  ...
  const handleRemoveImage = (field: keyof typeof previewImages, index?: number) => {
    if (field === "chequePhoto") {
      ...
      setVendorDetails((prev) => {
        ...
        console.log(`Removing image from ${field} at index ${index}:`, updatedField); // Debug log
  ...
  const handleSave = async () => {
    ...
    if (vendorDetails.businessName && !validateBusinessName(vendorDetails.businessName))
      return showPopup("error", "Business name must be at least 2 characters.");
    if (vendorDetails.phoneNumber && !validatePhoneNumber(vendorDetails.phoneNumber))
    ...
    try {
      ...
      const response = await axios.put(`${API_BASE_URL}/api/vendors/${vendorId}`, requestData, {
        withCredentials: true,
        headers: token ? { Authorization: `Bearer ${token}` } : {},
      });
  ...
  const renderVendorDetails = () => {
    ...
    if (!vendorDetails)
      return <div className="vendor-profile-form__loading">Failed to load vendor information</div>;
    ...
    return (
      <div className="vendor-profile-form">
        ...
        <div className="vendor-profile-form__row">
          <div className="vendor-profile-form__group vendor-profile-form__group--half">
            ...
            {isEditing ? (
              ...
              <div className="vendor-profile-form__display">{vendorDetails.businessName || "Not provided"}</div>
            )}
          </div>
          <div className="vendor-profile-form__group vendor-profile-form__group--half">
            <label>Email</label>
            <div className="vendor-profile-form__display">{vendorDetails.email || "Not provided"}</div>
          </div>
        </div>
        <div className="vendor-profile-form__row">
          <div className="vendor-profile-form__group vendor-profile-form__group--half">
            ...
            {isEditing ? (
              ...
              <div className="vendor-profile-form__display">{vendorDetails.phoneNumber || "Not provided"}</div>
            )}
          </div>
          <div className="vendor-profile-form__group vendor-profile-form__group--half">
            ...
            {isEditing ? (
              ...
              <div className="vendor-profile-form__display">{vendorDetails.businessAddress || "Not provided"}</div>
            )}
          </div>
        </div>
        <div className="vendor-profile-form__row">
          <div className="vendor-profile-form__group vendor-profile-form__group--half">
            ...
            {isEditing ? (
              ...
              <div className="vendor-profile-form__display">{vendorDetails.taxNumber || "Not provided"}</div>
            )}
          </div>
          <div className="vendor-profile-form__group vendor-profile-form__group--half">
            ...
            {isEditing ? (
              ...
              <div className="vendor-profile-form__display">{vendorDetails.businessRegNumber || "Not provided"}</div>
            )}
          </div>
        </div>
        <div className="vendor-profile-form__row">
          <div className="vendor-profile-form__group vendor-profile-form__group--half">
            ...
            {isEditing ? (
              ...
              <div className="vendor-profile-form__display">{vendorDetails.accountName || "Not provided"}</div>
            )}
          </div>
          <div className="vendor-profile-form__group vendor-profile-form__group--half">
            ...
            {isEditing ? (
              ...
              <div className="vendor-profile-form__display">{vendorDetails.bankName || "Not provided"}</div>
            )}
          </div>
        </div>
        <div className="vendor-profile-form__row">
          <div className="vendor-profile-form__group vendor-profile-form__group--half">
            ...
            {isEditing ? (
              ...
              <div className="vendor-profile-form__display">{vendorDetails.accountNumber || "Not provided"}</div>
            )}
          </div>
          <div className="vendor-profile-form__group vendor-profile-form__group--half">
            ...
            {isEditing ? (
              ...
              <div className="vendor-profile-form__display">{vendorDetails.bankBranch || "Not provided"}</div>
            )}
          </div>
        </div>
        <div className="vendor-profile-form__row">
          <div className="vendor-profile-form__group vendor-profile-form__group--half">
            ...
            {isEditing ? (
              ...
              <div className="vendor-profile-form__display">{vendorDetails.bankCode || "Not provided"}</div>
            )}
          </div>
        </div>
        <div className="vendor-profile-form__row">
          <div className="vendor-profile-form__group">
            ...
            {isEditing ? (
              <>
                ...
                >
                  <p>{isLoading.upload_chequePhoto ? "Uploading..." : "Click to upload cheque photo"}</p>
                </div>
                <input
                  type="file"
                  ref={chequeInputRef}
                ...
                <div className="vendor-profile-form__preview">
                  {previewImages.chequePhoto && (
                    <div className="vendor-profile-form__preview-item">
                      <img src={previewImages.chequePhoto} alt="Cheque Preview" />
                      <button
              ...
              <div className="vendor-profile-form__display">
                {vendorDetails.chequePhoto ? (
                  <img src={vendorDetails.chequePhoto} alt="Cheque" />
                ) : (
                  "Not provided"
                )}
              </div>
          ...
          <div className="vendor-profile-form__group">
            ...
            {isEditing ? (
              <>
                ...
                >
                  <p>{isLoading.upload_citizenshipDocuments ? "Uploading..." : "Click to upload citizenship documents"}</p>
                </div>
                <input
                  type="file"
                  ref={citizenshipInputRef}
                  multiple
                  onChange={(e) => handleArrayFileChange(e, "citizenshipDocuments")}
                  className="vendor-profile-form__input-hidden"
                  accept="image/*"
                />
                <div className="vendor-profile-form__preview">
                  {previewImages.citizenshipDocuments.map((src, index) => (
                    <div key={index} className="vendor-profile-form__preview-item">
                      <img src={src} alt="Citizenship Document Preview" />
                      <button
                        className="vendor-profile-form__preview-remove"
                        onClick={() => handleRemoveImage("citizenshipDocuments", index)}
                      >
                        ×
                      </button>
                    </div>
              ...
              <div className="vendor-profile-form__display">
                {vendorDetails.citizenshipDocuments?.length ? (
                  vendorDetails.citizenshipDocuments.map((doc, index) => (
                    <div key={index}>
                      <img src={doc} alt="Citizenship Document" />
                    </div>
                  ))
                ) : (
                  "Not provided"
          ...
          <div className="vendor-profile-form__group">
            ...
            {isEditing ? (
              <>
                ...
                >
                  <p>{isLoading.upload_taxDocuments ? "Uploading..." : "Click to upload tax documents"}</p>
                </div>
                <input
                  type="file"
                  ref={taxInputRef}
                ...
                <div className="vendor-profile-form__preview">
                  {previewImages.taxDocuments.map((src, index) => (
                    <div key={index} className="vendor-profile-form__preview-item">
                      <img src={src} alt="Tax Document Preview" />
                      <button
                        className="vendor-profile-form__preview-remove"
        ...
        {isEditing ? (
          ...
          <button className="vendor-btn-edit--primary" onClick={() => setIsEditing(true)}>
            Edit Profile
          </button>
        )}
      </div>
    ...
    return (
      <div className="vendor-credentials">
        ...
        <div className="vendor-credentials__header">
          <p className="vendor-credentials__description">Manage your password and account security</p>
        ...
        {credentialsMode === "forgot" && (
          <div className="vendor-credentials__section" style={{ display: "flex", flexDirection: "column", gap: "10px" }}>
            <h3>Reset Password</h3>
            <p>Enter your email address to receive a reset token.</p>
            <div className="vendor-profile-form__group">
              <label className="vendor-profile-form__label">Email Address</label>
              <div className="vendor-credentials__email-display">
                {formState.email || vendorDetails?.email || "No email available"}
              </div>
            </div>
            <button
        ...
        {credentialsMode === "reset" && (
          <div className="vendor-credentials__section">
            ...
            <div className="vendor-profile-form__group">
              <label className="vendor-profile-form__label">Confirm Password</label>
              <input
                type="password"
            ...
            <div className="vendor-credentials__actions-row">
              <button className="vendor-btn vendor-btn--secondary" onClick={() => setCredentialsMode("forgot")}>
                Back to Email
              </button>
              <button
                className="vendor-btn vendor-btn--primary"
      ...
      >
        <div className={`vendor-popup-content ${popup?.type}`}>
          <div className="vendor-popup-header">
            ...
            <span className="vendor-popup-title">{popup?.type === "success" ? "Success" : "Error"}</span>
          </div>
          <div className="vendor-popup-body">
            <p>{popup?.content}</p>
          </div>
          <button className="vendor-popup-close-btn" onClick={() => setPopup(null)}>
            Close
          </button>
        </div>
      </Popup>
        ...
        <div className="vendor-profile-main">
          ...
          <div
            className={`vendor-profile-card ${
              activeTab === "details" || activeTab === "credentials" ? "vendor-profile-card--wide" : ""
            }`}
          >
            <div className="vendor-profile-sidebar">
              {isLoading.fetchVendor ? (
                <>
                  ...
                  {[...Array(2)].map((_, i) => (
                    <div key={i} className="vendor-skeleton vendor-skeleton-button" />
                  ))}
                </>
              ) : (
                <>
            ...
            <div className="vendor-profile-content">{activeTab === "details" && renderVendorDetails()}</div>
            <div className="vendor-profile-content">{activeTab === "credentials" && renderCredentials()}</div>
          </div>
        </div>
      </div>
    </>
