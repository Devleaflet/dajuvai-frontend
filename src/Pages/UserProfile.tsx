...
const UserProfile: React.FC = () => {
  ...
  const [activeTab, setActiveTab] = useState<Tab>(() => {
    if (location.state && typeof location.state.activeTab === "string") {
      ...
      if (["details", "credentials", "orders"].includes(tab)) return tab as Tab;
    }
    return "details";
  });
  ...
  useEffect(() => {
    fetch("/Nepal-Address-API-main/data/provinces.json")
      .then((r) => r.json())
      .then((d) => setProvinceData(d.provinces.map(capitalizeFirstLetter)))
      .catch(console.error);
  }, []);
  ...
  const computeDeliveryTimeForOrder = (
    ...
    if (times.length && times.every((t) => t === "1-2 days")) return "1-2 days";
    return "3-5 days";
  };
  ...
  const ordersQuery = useQuery<OrderDetail[]>({
    ...
    queryFn: async () => {
      ...
      const res = await fetch(`${API_BASE_URL}/api/order/customer/history`, {
        credentials: "include",
        headers,
      });
  ...
  const vendorIds = useMemo(
    () => [
      ...new Set(
        orders.flatMap(
          (order) => order.orderItems?.map((i) => i.vendorId) ?? [],
        ),
      ),
    ],
    [orders],
    ...
    try {
      ...
      if (response.data.success) {
        ...
        const normalized: UserDetails = {
          ...updated,
          address: {
            province: updated.address?.province || updated.province || "",
            district: updated.address?.district || updated.district || "",
            city: updated.address?.city || updated.city || "",
            localAddress:
              updated.address?.localAddress || updated.localAddress || "",
            landmark: updated.address?.landmark || updated.landmark || "",
          },
        };
        setUserDetails(normalized);
        setOriginalDetails(normalized);
  ...
  const renderOrders = () => {
    ...
    if (ordersError) return <div className="orders__error">{ordersError}</div>;
    if (!orders.length)
                    ...
                    <div className="order-item__mobile-products">
                      {order.orderItems && order.orderItems.length > 0 ? (
                        (() => {
                          ...
                          const imgSrc =
                            first.imageSnapshot ||
                            first.variant?.variantImages?.[0] ||
                            first.product?.productImages?.[0];
                      ...
                      {(expandedOrders.has(order.id)
                        ...
                        return (
                          <div key={item.id} className="order-product">
                            ...
                            />
                            <span className="order-product__name">{name}</span>
                            <span className="order-product__quantity">
                              x{item.quantity}
                            </span>
                          </div>
            ...
            {isLoading.fetchUser ? (
              ...
              <>
                ...
                {(userDetails?.provider === "google"
                  ...
                  : (["details", "credentials", "orders"] as Tab[])
                ).map((tab) => (
                  <button
                    key={tab}
